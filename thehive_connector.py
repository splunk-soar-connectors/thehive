...
class ThehiveConnector(BaseConnector):
    ...
    def _handle_create_task(self, param):
        ...
        endpoint = f"api/case/{case_id}/task"
        headers = {"Content-Type": "application/json", "Authorization": self._auth_token}
        ret_val, response = self._make_rest_call(endpoint, action_result, params=None, data=data, headers=headers, method="post")
        if phantom.is_fail(ret_val):
            return action_result.get_status()
    ...
    def _handle_search_ticket(self, param, path="ticket"):
        ...
        data.update(search)
        headers = {"Content-Type": "application/json", "Authorization": self._auth_token}
        ret_val, response = self._make_rest_call(endpoint, action_result, params=params, data=data, headers=headers, method="post")
    ...
    def _handle_update_task(self, param):
        ...
        endpoint = f"api/case/task/{task_id}"
        headers = {"Content-Type": "application/json", "Authorization": self._auth_token}
    ...
    def _handle_get_observables(self, param):
        ...
        return action_result.set_status(phantom.APP_SUCCESS, f"Num observables found: {len(response_formatted)}")
    ...
    def _handle_create_observable(self, param):
        ...
        if data_type == "file":
            ...
            try:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR, f"Unable to find specified Vault file. Please check Vault ID and try again. {error_message}"
                )
        ...
        if ticket_type == "Ticket":
            endpoint = f"api/case/{case_id}/artifact"
        elif ticket_type == "Alert":
            endpoint = f"api/alert/{case_id}/artifact"
        else:
        ...
        if data_type == "file":
            ...
            ret_val, response = self._make_rest_call(
                endpoint, action_result, params=None, headers=headers, data=data, method="post", files=file_data
            )
        else:
            mesg["data"] = data
        ...
        if "failure" in response:
            ...
            message = "Error Type: {}. Error Message: {}".format(
                response.get("type", "Unknown"), response.get("message", "Unknown error occurred")
            )
    ...
    def _handle_create_task_log(self, param):
        ...
        endpoint = f"api/case/task/{task_id}/log"
        headers = {"Content-Type": "application/json", "Authorization": self._auth_token}
        ret_val, response = self._make_rest_call(endpoint, action_result, params=None, data=data, headers=headers, method="post")
        if phantom.is_fail(ret_val):
            return action_result.get_status()
    ...
    def _handle_add_ttp(self, param):
        ...
        if param.get("occur_date"):
            try:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR, "Please provide a valid date in the 'occur_date' parameter, supported format: DD-MM-YYYY hh:mm"
                )
        else:
            date_time_obj = datetime.datetime.now()

        data.update({"caseId": ticket_id, "patternId": pattern_id, "tactic": tactic, "occurDate": int(date_time_obj.timestamp() * 1000)})
